...
from enas_operations import OPERATIONS, OPERATIONS_large, OPERATIONS_large_name, MaybeCalibrateSize, FactorizedReduce, AuxHeadCIFAR, apply_drop_path, FinalCombine, BatchNorm, ReLU, Linear, Conv2d
...
class NASNetworkCIFAR(AbstractClassifier):
    def __init__(self, args, classes, layers, nodes, channels, keep_prob, drop_path_keep_prob, use_aux_head, steps, arch):
        ...
        self.stem = nn.Sequential(
            Conv2d(3, channels, 3, padding=1, bias=False),
            BatchNorm(channels)
        )
        ...
        self.classifier = Linear(outs[-1][-1], classes)
    ...
    def forward(self, input, step=None):
        ...
        if aux_logits is not None:
            return F.log_softmax(logits, dim=-1), F.log_softmax(aux_logits, dim=-1)
        return F.log_softmax(logits, dim=-1)


def make_random_cell(size=5, random=None):
    if random is None:
        random = np.random
    cell = []
    for layer_id in range(2, size + 2):
        idx_a = random.randint(layer_id)
        idx_b = random.randint(layer_id)
        op_a = random.choice(list(OPERATIONS_large.keys()))
        op_b = random.choice(list(OPERATIONS_large.keys()))
        cell.extend((idx_a, op_a, idx_b, op_b))
    return cell


def sample_enas(cell_size=5, seed=None):
    if seed is None:
        random = np.random
    else:
        random = np.random.RandomState(seed)
    cell = make_random_cell(cell_size, random=random)
    reduc_cell = make_random_cell(cell_size, random=random)
    return [cell, reduc_cell]


def cell_to_dag(cell):
    conv_dag = OrderedDict()
    conv_dag['node_1'] = ['node_1', None, None, None, None]
    conv_dag['node_2'] = ['node_2', None, None, None, None]
    cell_size = len(cell) // 4
    assert len(cell) % 4 == 0
    for i in range(0, cell_size):
        idx_a, op_a, idx_b, op_b = cell[4 * i:4 * i + 4]
        name = 'node_{}'.format(i + 3)
        first_node = 'node_{}'.format(idx_a + 1)
        second_node = 'node_{}'.format(idx_b + 1)
        first_op = OPERATIONS_large_name[op_a]
        second_op = OPERATIONS_large_name[op_b]
        conv_dag[name] = [name, first_node, second_node, first_op, second_op]
    return conv_dag


def encoding_to_dag(cells):
    conv_cell, reduc_cell = cells
    return {"conv_dag": cell_to_dag(conv_cell), "reduc_dag": cell_to_dag(reduc_cell)}
